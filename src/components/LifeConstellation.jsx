...
import { Music, CreditCard, MapPin, FileText, Sparkles, Receipt, ArrowRight } from 'lucide-react'
...
export function LifeConstellation({ onSelectReceipt, onOpenGraph }) {
  const [activeNode, setActiveNode] = useState(NODE_TYPES[4]) // default Life Moment
        ...
        {NODE_TYPES.map((node, idx) => {
          ...
          return (
            <motion.div
              ...
              onClick={() => {
                setActiveNode(node)
                if (onSelectReceipt) onSelectReceipt(node.receiptId)
              }}
      ...
      {activeNode && (
        <div className="mt-4 p-4 rounded-xl bg-dark-bg/90 border border-dark-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 z-10 relative">
          <div className="flex items-center gap-3">
            <div 
              className="w-8 h-8 rounded-lg flex items-center justify-center text-emerald-400 bg-emerald-500/10 border border-emerald-500/30"
            >
            ...
            <div>
              ...
              <div className="text-xs text-gray-400">
                Connected to 3 other life receipts occurring on November 12, 2024.
              </div>
            </div>
          </div>

          <button
            onClick={() => onSelectReceipt && onSelectReceipt(activeNode.receiptId)}
